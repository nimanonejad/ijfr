...
ir=0; %remember to change ir=0 high activity
% ir=1; %remember to change ir=1 low activity
mquant=zeros(19,3);
        ...
        for i=1:size(mpyt,1)
            ...
            if mean(vpm,2)==vpm(1,1)
               ...
               vpm=(mpyt(i,:).*vpm)./vsum;
                ...
                mrec=xlsread('usrec.xls');
                ibx=199001;
                iex=202412;
                %vdum=mrec(find(mrec(:,end)==ibx):find(mrec(:,end)==iex),1);
                vdum=(mrec(find(mrec(:,end)==ibx):find(mrec(:,end)==iex),end-1)<0)*1;
                vyc=vys(is:ie,1);
                mcc=[vetab(is:ie,1),vetac(is:ie,:)];
                myc=[vyhab(is:ie,1),vyhac(is:ie,:)];
                mcq=mcc(vdum==ir,:);
                myq=myc(vdum==ir,:);
                vyq=vyc(vdum==ir,:);
                mquant(k,:)=[mean(mcq(:,2),1)./mean(mcq(:,1),1),funcDM([mcq(:,1),mcq(:,2)],1),funcCW(myq(:,1),myq(:,2),vyq)];
