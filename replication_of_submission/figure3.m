        ...
        for i=1:size(mpyt,1)
            ...
            if mean(vpm,2)==vpm(1,1)
               ...
               vpm=(mpyt(i,:).*vpm)./vsum;
                ...
                mquant(k,1:2)=[mean(vlosc(is:ie,:),1)./mean(vlosb(is:ie,1),1),funcDM([vlosb(is:ie,1),vlosc(is:ie,1)],1)];
                mquant(k,3:4)=[sqrt(12)*(mean(vDRc(is:ie,:),1))./std(vDRc(is:ie,:),1),funcDM([-1*vDRc(is:ie,1),vDRb(is:ie,1)],1)];                
end

vro=squeeze(amqn(is:ie,13,end));
j=1;
vroc=zeros(size(0:12:size(vro,1),2),2);
for i=0:12:size(vro,1);
    if i>0
    vroc(j,:)=vro(i,:);
    j=j+1;
    end
